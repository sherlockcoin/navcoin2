...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // due to a new block arriving or other large navcoin.
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "NavCoin " + FormatFullVersion();
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. NavCoin is probably already running."), addrBind.ToString());
        else
...
void RelayAnonSendIn(const std::vector<CTxIn>& in, const int64_t& nAmount, const CTransaction& txCollateral, const std::vector<CTxOut>& out)
{
    ...
    BOOST_FOREACH(CNode* pnode, vNodes)
    {
        if((CNetAddr)anonSendPool.submittedToInode != (CNetAddr)pnode->addr) continue;
        LogPrintf("RelayAnonSendIn - found master, relaying message - %s \n", pnode->addr.ToString().c_str());
